...
import type { WorkflowPathTraceEntry } from "../../shared/runtime";
...
const kindClassName: Record<"dark" | "light", Record<NodeKind, string>> = {
  dark: {
    ...
    condition:
      "border-fuchsia-300/25 bg-[linear-gradient(180deg,rgba(48,19,60,0.92),rgba(27,10,36,0.94))] text-fuchsia-50",
    tool:
      "border-cyan-300/25 bg-[linear-gradient(180deg,rgba(14,45,58,0.92),rgba(9,24,31,0.94))] text-cyan-50",
    review:
      "border-amber-300/25 bg-[linear-gradient(180deg,rgba(53,35,12,0.92),rgba(31,20,7,0.94))] text-amber-50",
    loop:
      "border-violet-300/25 bg-[linear-gradient(180deg,rgba(35,22,58,0.92),rgba(18,10,35,0.94))] text-violet-50",
    end:
  ...
  light: {
    ...
    condition:
      "border-fuchsia-500/25 bg-[linear-gradient(180deg,rgba(248,235,251,0.98),rgba(238,223,246,0.95))] text-fuchsia-950",
    tool:
      "border-cyan-500/25 bg-[linear-gradient(180deg,rgba(230,246,249,0.98),rgba(214,237,241,0.95))] text-cyan-950",
    review:
      "border-amber-500/25 bg-[linear-gradient(180deg,rgba(252,243,224,0.98),rgba(246,231,198,0.95))] text-amber-950",
    loop:
      "border-violet-500/25 bg-[linear-gradient(180deg,rgba(239,236,253,0.98),rgba(226,220,248,0.95))] text-violet-950",
    end:
...
export const collectActiveEdgeIds = (pathTrace: WorkflowPathTraceEntry[]): string[] =>
  pathTrace
    .map((entry) => entry.chosenExit?.edgeId)
...
export const WorkflowCanvasV2 = ({
  ...
  const edgeViews = useMemo<EdgeView[]>(() => {
    return edges
      ...
      .filter((edge): edge is EdgeView => Boolean(edge));
  }, [edges, nodeMap]);
  ...
  return (
    <div
      className={`relative h-[820px] overflow-hidden rounded-[28px] border ${
        theme === "dark"
    ...
    >
      ...
      <svg className="absolute inset-0 h-full w-full">
        ...
        {edgeViews.map((edge) => {
          ...
          const displayLabel = edge.label || edge.branchKey || "";
            ...
            <g key={edge.id}>
              <path
                ...
                stroke={
                  isActive
                    ? theme === "dark"
                      ? "rgba(91, 235, 193, 0.95)"
                      : "rgba(22, 163, 74, 0.9)"
                    : theme === "dark"
                      ? "rgba(216, 163, 93, 0.8)"
                      : "rgba(166, 112, 39, 0.72)"
                }
